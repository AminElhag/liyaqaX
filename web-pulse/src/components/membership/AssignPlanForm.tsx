...
import { z } from 'zod'
import { assignMembership, membershipKeys } from '@/api/memberships'
...
export function AssignPlanForm({ memberId, onSuccess, onCancel }: AssignPlanFormProps) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      onSuccess(data.invoice?.invoiceNumber ?? '')
    },
  })
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="flex justify-end gap-3 pt-2">
        ...
        >
          {isSubmitting ? t('common.saving') : t('membership.assignAndCollect')}
        </button>
      </div>
    </form>
  )
